...
import type {ModuleLogger} from '@main/dav/runtimeContracts';
...
type CloudDavDependencies = {
	cloudAccountToDavAccountId: (accountId: number) => number;
	getDavSettings: (accountId: number) => {carddav_url: string | null; caldav_url: string | null} | null;
	upsertDavSettings: (accountId: number, carddavUrl?: string | null, caldavUrl?: string | null) => unknown;
	upsertContacts: (
		accountId: number,
		rows: Array<{
			sourceUid: string;
	...
	createLogger: (channel: 'carddav' | 'caldav', context: string) => ModuleLogger;
};

const notConfigured = () => {
	throw new Error('Cloud DAV runtime dependencies are not configured.');
};

let dependencies: CloudDavDependencies = {
	cloudAccountToDavAccountId: notConfigured as CloudDavDependencies['cloudAccountToDavAccountId'],
	getDavSettings: notConfigured as CloudDavDependencies['getDavSettings'],
	upsertDavSettings: notConfigured as CloudDavDependencies['upsertDavSettings'],
	upsertContacts: notConfigured as CloudDavDependencies['upsertContacts'],
	upsertCalendarEvents: notConfigured as CloudDavDependencies['upsertCalendarEvents'],
	createLogger: notConfigured as CloudDavDependencies['createLogger'],
};

export function configureCloudDavDependencies(nextDependencies: CloudDavDependencies): void {
...
export async function syncCloudDav(account: CloudAccountCredentials): Promise<CloudDavSyncSummary> {
	if (account.provider !== 'nextcloud' && account.provider !== 'webdav') {
		return {
			...
			davAccountId: dependencies.cloudAccountToDavAccountId(account.id),
			carddavUrl: null,
			caldavUrl: null,
	...
	const davAccountId = dependencies.cloudAccountToDavAccountId(account.id);
	const loggerCarddav = dependencies.createLogger('carddav', `cloud:${account.id}`);
	const loggerCaldav = dependencies.createLogger('caldav', `cloud:${account.id}`);

	const saved = dependencies.getDavSettings(davAccountId);
	const carddavUrl =
	...
	dependencies.upsertDavSettings(davAccountId, carddavUrl, caldavUrl);
	...
	if (carddavUrl) {
		...
		contactsResult = {
			...dependencies.upsertContacts(davAccountId, contacts, 'cloud-carddav'),
	...
	if (caldavUrl) {
		...
		eventsResult = {
			...dependencies.upsertCalendarEvents(davAccountId, events, 'cloud-caldav'),
...
async function discoverHomeUrl(
	...
	const configured = parseConfiguredCloudBaseUrl(creds.baseUrl);
	const origin = configured.origin;
	const basePath = ensureTrailingSlash(configured.pathname || '/');
	const candidates = dedupe([
		`${origin}/.well-known/${kind}`,
		`${origin}${basePath}.well-known/${kind}`,
		`${origin}/remote.php/dav/`,
		`${origin}${basePath}remote.php/dav/`,
		`${origin}/dav/`,
		`${origin}${basePath}dav/`,
		ensureTrailingSlash(configured.toString()),
	]);
...
function parseConfiguredCloudBaseUrl(rawBaseUrl: string): URL {
	const normalized = String(rawBaseUrl || '').trim();
	if (!normalized) throw new Error('Cloud DAV base URL is missing.');
