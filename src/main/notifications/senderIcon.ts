...
function extractSenderDomain(fromAddress: string | null | undefined): string | null {
    const raw = String(fromAddress || '').trim().toLowerCase();
    if (!raw) return null;
...
export async function resolveSenderNotificationIconPath(fromAddress: string | null | undefined): Promise<string | undefined> {
