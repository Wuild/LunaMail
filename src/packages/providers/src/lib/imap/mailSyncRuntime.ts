...
type MailSyncRuntimeDependencies = {
	...
	resolveImapSecurity: (imapSecure: number) => Record<string, unknown>;
	resolveImapAuth: (account: ProviderMailAuthCredentials) => Promise<ProviderImapAuth> | ProviderImapAuth;
	...
	getFolderPositiveUidBounds: (...args: any[]) => any;
	getMessageBody: (...args: any[]) => any;
	getMessageContext: (...args: any[]) => any;
	getMessageIdByFolderAndUid: (...args: any[]) => any;
	hasMessageByFolderAndUid: (...args: any[]) => any;
	listFoldersByAccount: (...args: any[]) => any;
	listMessageAttachments: (...args: any[]) => any;
	reconcileFolderMessageUids: (...args: any[]) => any;
	replaceMessageAttachments: (...args: any[]) => any;
	updateFolderCounts: (...args: any[]) => any;
	upsertFolder: (...args: any[]) => any;
	upsertMessage: (...args: any[]) => any;
	upsertMessageBody: (...args: any[]) => any;
	upsertThread: (...args: any[]) => any;
	buildThreadId: (...args: any[]) => any;
	stringifyReferences: (...args: any[]) => any;
	resolveDriverForAccount: (accountId: number) => Promise<{
		key: () => string;
		supports: (capability: ProviderCapability) => boolean;
		canRunIncrementalSync: () => boolean;
		resolveSyncCredentials: (accountId: number) => Promise<{
			id: number;
			imap_host: string;
			imap_port: number;
			imap_secure: number;
...
const notConfigured = () => {
	throw new Error('IMAP mail sync runtime dependencies are not configured.');
};

let createMailDebugLogger: MailSyncRuntimeDependencies['createMailDebugLogger'] = notConfigured as any;
let resolveImapSecurity: MailSyncRuntimeDependencies['resolveImapSecurity'] = notConfigured as any;
let resolveImapAuth: MailSyncRuntimeDependencies['resolveImapAuth'] = notConfigured as any;
let getAccountSyncCredentials: MailSyncRuntimeDependencies['getAccountSyncCredentials'] = notConfigured as any;
let getFolderPositiveUidBounds: MailSyncRuntimeDependencies['getFolderPositiveUidBounds'] = notConfigured;
let getMessageBody: MailSyncRuntimeDependencies['getMessageBody'] = notConfigured;
let getMessageContext: MailSyncRuntimeDependencies['getMessageContext'] = notConfigured;
let getMessageIdByFolderAndUid: MailSyncRuntimeDependencies['getMessageIdByFolderAndUid'] = notConfigured;
let hasMessageByFolderAndUid: MailSyncRuntimeDependencies['hasMessageByFolderAndUid'] = notConfigured;
let listFoldersByAccount: MailSyncRuntimeDependencies['listFoldersByAccount'] = notConfigured;
let listMessageAttachments: MailSyncRuntimeDependencies['listMessageAttachments'] = notConfigured;
let reconcileFolderMessageUids: MailSyncRuntimeDependencies['reconcileFolderMessageUids'] = notConfigured;
let replaceMessageAttachments: MailSyncRuntimeDependencies['replaceMessageAttachments'] = notConfigured;
let updateFolderCounts: MailSyncRuntimeDependencies['updateFolderCounts'] = notConfigured;
let upsertFolder: MailSyncRuntimeDependencies['upsertFolder'] = notConfigured;
let upsertMessage: MailSyncRuntimeDependencies['upsertMessage'] = notConfigured;
let upsertMessageBody: MailSyncRuntimeDependencies['upsertMessageBody'] = notConfigured;
let upsertThread: MailSyncRuntimeDependencies['upsertThread'] = notConfigured;
let buildThreadId: MailSyncRuntimeDependencies['buildThreadId'] = notConfigured;
let stringifyReferences: MailSyncRuntimeDependencies['stringifyReferences'] = notConfigured;
let resolveDriverForAccount: MailSyncRuntimeDependencies['resolveDriverForAccount'] = notConfigured as any;

export function configureImapMailSyncDependencies(dependencies: MailSyncRuntimeDependencies): void {
	createMailDebugLogger = dependencies.createMailDebugLogger;
	resolveImapSecurity = dependencies.resolveImapSecurity;
	resolveImapAuth = dependencies.resolveImapAuth;
	getAccountSyncCredentials = dependencies.getAccountSyncCredentials;
	getFolderPositiveUidBounds = dependencies.getFolderPositiveUidBounds;
	getMessageBody = dependencies.getMessageBody;
	getMessageContext = dependencies.getMessageContext;
	getMessageIdByFolderAndUid = dependencies.getMessageIdByFolderAndUid;
	hasMessageByFolderAndUid = dependencies.hasMessageByFolderAndUid;
	listFoldersByAccount = dependencies.listFoldersByAccount;
	listMessageAttachments = dependencies.listMessageAttachments;
	reconcileFolderMessageUids = dependencies.reconcileFolderMessageUids;
	replaceMessageAttachments = dependencies.replaceMessageAttachments;
	updateFolderCounts = dependencies.updateFolderCounts;
	upsertFolder = dependencies.upsertFolder;
	upsertMessage = dependencies.upsertMessage;
	upsertMessageBody = dependencies.upsertMessageBody;
	upsertThread = dependencies.upsertThread;
	buildThreadId = dependencies.buildThreadId;
	stringifyReferences = dependencies.stringifyReferences;
	resolveDriverForAccount = dependencies.resolveDriverForAccount;
}
...
export async function syncAccountMailbox(accountId: number, options?: AccountSyncOptions): Promise<SyncSummary> {
	const driver = await resolveDriverForAccount(accountId);
...
export async function syncMessageBody(messageId: number, options?: MessageBodySyncOptions): Promise<MessageBodyResult> {
	...
	if (cached && (cached.text_content || cached.html_content)) {
		const attachments = listMessageAttachments(messageId).map((attachment: any) => ({
