...
export default function SettingsAccountFiltersPage() {
    ...
    return (
        <>
            <section className="panel rounded-xl p-4">
                <div className="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p className="mt-1 ui-text-muted text-sm">Thunderbird-style account filters that can run on new
                            incoming mail or manually.</p>
                    </div>
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
                                        <span
                                            className={cn('rounded px-2 py-0.5', filter.enabled ? 'chip-success' : 'ui-surface-hover ui-text-secondary')}>
            ...
            {mailFilterModal && (
                <Modal open onClose={() => setMailFilterModal(null)} backdropClassName="z-[1200]"
                    ...
                    <header className="ui-border-default flex items-start justify-between gap-3 border-b px-5 py-4">
                        <div className="min-w-0">
                            <h3 className="ui-text-primary text-lg font-semibold">{mailFilterModal.mode === 'create' ? 'Create Filter' : 'Edit Filter'}</h3>
                            <p className="ui-text-muted mt-1 text-sm">Configure matching rules and actions for this
                                account.</p>
                        </div>
                        <Button type="button" variant="secondary" size="sm" className="rounded-md px-2"
                                onClick={() => setMailFilterModal(null)} disabled={mailFilterBusy}>
                            Close
                        </Button>
                    </header>
                    ...
                    <div className="max-h-[72vh] space-y-4 overflow-y-auto px-5 py-4">
                        <section className="panel rounded-xl p-4">
                            <div className="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-end">
                                <label className="block text-sm">
                                    ...
                                    <FormInput value={mailFilterModal.draft.name}
                                ...
                                <label
                                    ...
                                    <FormCheckbox checked={mailFilterModal.draft.enabled}
                                                  onChange={(e) => updateMailFilterDraft((prev) => ({
                                                      ...prev,
                                ...
                                <label
                                    className="ui-border-default inline-flex h-12 items-center gap-2 rounded-lg border px-3 text-sm ui-text-secondary">
                                    <FormCheckbox checked={mailFilterModal.draft.run_on_incoming}
                                                  onChange={(e) => updateMailFilterDraft((prev) => ({
                                                      ...prev,
                        ...
                        <section className="panel rounded-xl p-4">
                            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                                ...
                                <label
                                    className="ui-border-default inline-flex h-12 items-center gap-2 rounded-lg border px-3 text-sm ui-text-secondary">
                                    <FormCheckbox checked={mailFilterModal.draft.stop_processing}
                                                  onChange={(e) => updateMailFilterDraft((prev) => ({
                                                      ...prev,
                                    ...
                                    {mailFilterModal.draft.conditions.map((condition, index) => (
                                        <div key={index}
                                            ...
                                            <FormInput
                                                ...
                                                placeholder="Value"
                                            />
                                            <Button type="button" variant="danger" size="sm" className="rounded-md px-2"
                                                    onClick={() => updateMailFilterDraft((prev) => ({
                                                        ...prev,
                                ...
                                {mailFilterModal.draft.actions.map((action, index) => (
                                    <div key={index}
                                        ...
                                        {action.type === 'move_to_folder' ? (
                                            ...
                                            <FormInput type="text" disabled value="" variant="subtle" size="lg"
                    ...
                    <footer className="ui-border-default flex items-center justify-end gap-2 border-t px-5 py-4">
                        <Button type="button" variant="secondary" size="default" className="rounded-md px-4"
                                onClick={() => setMailFilterModal(null)} disabled={mailFilterBusy}>
                            Cancel
                        </Button>
                        <Button type="button" variant="default" size="default"
                                onClick={() => void onSaveMailFilterModal()} disabled={mailFilterBusy}
                                className="rounded-md px-4 font-medium disabled:opacity-50">
                            {mailFilterBusy ? 'Saving...' : mailFilterModal.mode === 'create' ? 'Create Filter' : 'Save Filter'}
                        </Button>
                    </footer>
                </Modal>
            )}
