...
export function isSenderAllowed(fromAddress: string | null | undefined, allowlist: string[]): boolean {
    ...
    const normalized = new Set(
        (allowlist || []).map((entry) => normalizeAllowlistEntry(entry)).filter((entry): entry is string => Boolean(entry))
    );
    if (normalized.has(senderEmail)) return true;
    return !!(senderDomain && normalized.has(senderDomain));
}
...
export function buildSourceDocCsp(allowRemote: boolean): string {
    ...
    const styleSources = allowRemote ? "'unsafe-inline' https: http: data:" : "'unsafe-inline'";
    const baseUri = allowRemote ? "https: http:" : "'none'";
    return [
        "default-src 'none'",
        `base-uri ${baseUri}`,
        "form-action 'none'",
