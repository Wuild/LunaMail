...
function MailPage() {
  ...
  const syncReadStateRef = useRef<(message: MessageItem, nextRead: number, folderPath: string | null) => Promise<void>>(
      async () => undefined
  );
  const loadFoldersAndMessagesRef = useRef<
      (accountId: number, preferredFolderId?: number | null, preferredMessageId?: number | null) => Promise<void>
  >(async () => undefined);
  const keyboardActionsRef = useRef<{
    confirmAndDeleteMessage: (message: MessageItem) => void;
    navigateMessageSelection: (direction: 1 | -1, extendSelection?: boolean) => void;
    onForward: () => void;
    onRefresh: () => Promise<void>;
    onReply: () => void;
    onReplyAll: () => void;
  }>({
    confirmAndDeleteMessage: () => undefined,
    navigateMessageSelection: () => undefined,
    onForward: () => undefined,
  ...
  const accountFoldersByIdRef = useRef<Record<number, FolderItem[]>>({});
...
<html>
  ...
  useEffect(() => {
    ...
    setPendingAutoReadMessageId(null);
    void syncReadStateRef.current(selectedMessage, 1, selectedFolderPath);
  ...
  useEffect(() => {
    accountFoldersByIdRef.current = accountFoldersById;
  }, [accountFoldersById]);
  ...
  useEffect(() => {
    ...
    const offSync = window.electronAPI.onAccountSyncStatus?.((evt: SyncStatusEvent) => {
      ...
      if (evt.status === "syncing") {
        ...
        if (evt.accountId === currentSelectedAccountId || currentSelectedAccountId === null) {
        ...
        if (evt.accountId === currentSelectedAccountId) {
          if (hasPendingReadForAccount(evt.accountId) || hasRecentLocalReadMutation(evt.accountId)) return;
          void loadFoldersAndMessagesRef.current(evt.accountId);
        }
    ...
    const offOpenMessageTarget = window.electronAPI.onOpenMessageTarget?.((target) => {
      ...
      const accountFolders = accountFoldersByIdRef.current[target.accountId] ?? [];
      ...
      if (matchedFolder) {
        ...
        if (locationPathnameRef.current !== targetPath) {
          navigate(targetPath);
        }
      } else {
        const fallbackPath = `/email/${target.accountId}`;
        if (locationPathnameRef.current !== fallbackPath) {
  ...
  useEffect(() => {
    ...
    void loadFoldersAndMessagesRef.current(
        selectedAccountId,
        routeAccountId === selectedAccountId ? routeFolderId : null,
        null
    );
  }, [selectedAccountId, routeAccountId, routeFolderId]);
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      try {
        const rowsRaw = await window.electronAPI.getFolderMessages(
            selectedAccountId,
            selectedFolderPath,
            messageFetchLimit
        );
  ...
  async function syncReadState(message: MessageItem, nextRead: number, folderPath: string | null) {
    ...
    try {
      ...
      setMessages((prev) => prev.map((m) => (m.id === message.id ? {...m, is_read: res.isRead} : m)));
  ...
  syncReadStateRef.current = syncReadState;
  loadFoldersAndMessagesRef.current = loadFoldersAndMessages;
  ...
  function handleSelectMessage(
    ...
    if (toggleKey) {
      ...
      const nextIds = exists ? selectedMessageIds.filter((x) => x !== id) : [...selectedMessageIds, id];
  ...
  function navigateMessageSelection(direction: 1 | -1, extendSelection = false): void {
    ...
    const currentIndex = selectedMessageId ? messages.findIndex((message) => message.id === selectedMessageId) : -1;
  ...
  function runAttachmentAction(index: number, action: "open" | "save") {
    if (!selectedMessage) return;
    void window.electronAPI.openMessageAttachment(selectedMessage.id, index, action).catch((error: any) => {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (key === "delete" && !event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) {
        if (!selectedMessage) return;
        event.preventDefault();
        actions.confirmAndDeleteMessage(selectedMessage);
        return;
      }
      ...
      if (!mod && !event.altKey && key === "arrowdown") {
        event.preventDefault();
        actions.navigateMessageSelection(1, event.shiftKey);
        return;
      }
      ...
      <MainLayout
          ...
          onBulkMove={(messageIds, targetFolderPath) =>
              void (() => {
                ...
                for (const message of targets) {
                  void window.electronAPI
                      .moveMessage(message.id, targetFolderPath)
                      .then((res) => {
                        setFolders((prev) =>
                            prev.map((folder) => {
                              if (folder.id === res.sourceFolderId) {
              ...
              <section className="h-full overflow-auto bg-slate-50 p-5 dark:bg-[#26292f]">
                <div className="mx-auto max-w-5xl">
                  ...
                  {selectedAccountId && (
                      <>
                        {calendarLoading && (
                            <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">Loading events...</p>
                        )}
                        {!calendarLoading && calendarEvents.length === 0 && (
                            <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">No events found.</p>
                        )}
                        {!calendarLoading && calendarEvents.length > 0 && (
                            <div
          ...
          {workspace === "mail" && selectedMessage && (
              <article className="flex h-full flex-col">
                ...
                >
                  ...
                  <ToolboxButton label="Open" icon={<SquareArrowOutUpRight size={14}/>} onClick={onOpenInNewWindow}/>
                  <ToolboxButton label="View source" icon={<FileText size={14}/>} onClick={onViewSource}/>
                  <ToolboxButton label="Delete" icon={<Trash2 size={14}/>} onClick={onDeleteSelected} danger/>
                </div>
                <div
                  ...
                  {showMessageDetails && (
                      <div
                        ...
                        <div>
                          <span className="font-medium">From name:</span> {selectedMessage.from_name || "-"}
                        </div>
                        <div>
                          <span className="font-medium">From address:</span> {selectedMessage.from_address || "-"}
                        </div>
                        <div>
                        ...
                        <div>
                          <span className="font-medium">Message-ID:</span> {selectedMessage.message_id || "-"}
                        </div>
                        <div>
                          <span className="font-medium">In-Reply-To:</span> {selectedMessage.in_reply_to || "-"}
                        </div>
                        <div>
                          <span className="font-medium">References:</span> {selectedMessage.references_text || "-"}
                        </div>
                        <div>
                ...
                <div className="min-h-0 flex flex-1 flex-col bg-white">
                  {Boolean(
                      renderedBodyHtml && selectedMessage && appSettings.blockRemoteContent && !allowRemoteForSelectedMessage
                  ) && (
                      <div
                    ...
                    {isMailBodyViewportTooSmall && (
                        <div
                          ...
                          <div className="max-w-md text-sm text-slate-600 dark:text-slate-300">
                            <p>
                              Preview is too small ({mailBodyViewport.width}x{mailBodyViewport.height}). Message opened
                              in a
                              separate window.
                            </p>
                            <button
              ...
              >
                ...
                <div className="min-h-0 flex-1 overflow-auto bg-slate-50 p-3 dark:bg-[#181a1f]">
                  ...
                  {!sourceLoading && sourceError && (
                      <p className="text-sm text-red-700 dark:text-red-300">Failed to load source: {sourceError}</p>
                  )}
                  {!sourceLoading && !sourceError && (
                      <pre
